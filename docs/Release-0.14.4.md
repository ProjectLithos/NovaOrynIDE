...
0.14.4 removes the long inline PowerShell expressions used for IDE build-state invalidation and validation.
...
## 0.14.4 build-script parser fix

Top-level Build and Run batch files no longer use PowerShell `-Command`. Package-lock maintenance and runtime package validation are delegated to dedicated `.ps1` files invoked with `-File`, preventing CMD from reparsing VERSION manifest text after a quoting failure.
