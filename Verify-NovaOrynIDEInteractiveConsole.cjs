...
let bad=false;for(const [n,ok] of checks){console.log(`${ok?'[ OK ]':'[FAIL]'} ${n}`);if(!ok)bad=true;}if(bad)process.exit(1);console.log(`[ OK ] NovaOryn IDE 0.9.0 interactive console/input bridge verified (${checks.length} checks).`);
