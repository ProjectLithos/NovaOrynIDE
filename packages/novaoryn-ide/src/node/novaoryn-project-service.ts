...
const NOVAORYN_IDE_VERSION = '0.10.11';
...
export class NovaOrynProjectServiceImpl implements NovaOrynProjectService {
    ...
    async deleteOperatingSystemSource(projectPath: string): Promise<NovaOrynProjectResult> {
        try {
            ...
            // Delete source first. A successful return from this method therefore means
            // the OS can no longer be discovered by listOperatingSystems().
            await fs.rm(projectRoot, { recursive: true, force: false });

            // Remove any stale list metadata for the deleted instance while deliberately
            // preserving nextInstanceByName so creation numbers never go backwards.
            const state = await this.readOperatingSystemListState();
            const key = this.normalizedOsPath(projectRoot);
    ...
    async runOperatingSystem(projectPath: string, mode: NovaOrynRunMode, breakpoints: NovaOrynBreakpointRequest[] = [], exceptionBreakpoints: NovaOrynExceptionBreakpointSettings = { vectors: [0, 2, 6, 8, 12, 13, 14, 18], breakOnPanic: true }): Promise<NovaOrynRunResult> {
        try {
            ...
            if (activeTarget.kind === 'remote') return { success: false, error: `${activeTarget.name} is a remote target. NovaOryn IDE 0.10.11 implements direct physical-machine GDB transport; generic remote-agent execution remains reserved for a later transport.` };
        ...
        if (session.serialLogPath) {
            try {
                ...
                if (serial.length > serialOffset) {
                    ...
                    if (fresh.includes('[[NOVAORYN:SHELL_COPY_ALL]]')) {
                        const copyMarker = serial.lastIndexOf('[[NOVAORYN:SHELL_COPY_ALL]]');
                        const beginMarker = serial.lastIndexOf('[[NOVAORYN:SHELL_BEGIN]]', copyMarker);
                        if (beginMarker >= 0 && copyMarker > beginMarker) {
                            const transcriptStart = beginMarker + '[[NOVAORYN:SHELL_BEGIN]]'.length;
                            ...
                            if (transcript.length > 0) {
                                ...
                                session.output += '\r\n[ OK ] NovaOryn shell transcript copied to Windows clipboard (Ctrl+A, Ctrl+C).\r\n';
                            }
                        }
                    }
        ...
        try {
            ...
            if (diagnosticHeader.length === 64 && diagnosticHeader.readBigUInt64LE(0) === diagnosticMagic && diagnosticHeader.readUInt32LE(8) === 1 && diagnosticHeader.readUInt32LE(12) === 512) {
                ...
                if (!stateSymbol || session.relocationDelta === undefined) {
                    return { success: false, error: 'This kernel predates the stable KernelHeap diagnostic ABI and NativeAOT did not expose its private _state symbol. Rebuild the OS with the bundled NovaOryn SDK from IDE 0.10.11 or later.' };
                }
    ...
    async loadCrashDump(dumpPath: string): Promise<NovaOrynCrashDumpResult> {
        try {
            ...
            // Pre-0.10.11 IDE dumps are retained as a documented legacy import path.
    ...
    protected ingestStructuredTelemetry(session: RunSession, line: string, now: number): boolean {
        const match = /^\[NOVAORYN:(TRACE|BOOT|PROFILE|COUNTER|DIAGNOSTIC)\]\s*(.*)$/i.exec(line); if (!match) return false;
    ...
    protected async refreshSdkBridge(projectRoot: string): Promise<void> {
        ...
        // Repair versioned generated bootstrap support files in existing OSes. These are SDK-owned,
        // not user-editable kernel source, so keeping them synchronized is safe and required.
        const generatedTemplateRoot = path.join(NOVAORYN_SDK_ROOT, 'templates', 'NovaOrynKernel');
        for (const relative of [
            path.join('Boot', 'BootStartup.cs'),
            path.join('Boot', 'KernelStructuredLogging.cs'),
            path.join('Boot', 'KernelTelemetryTransport.cs'),
            path.join('Boot', 'KernelPanicTransport.cs')
        ]) {
            const source = path.join(generatedTemplateRoot, relative);
            const destination = path.join(projectRoot, relative);
            await fs.mkdir(path.dirname(destination), { recursive: true });
            await fs.copyFile(source, destination);
        }

        const kernelProjectTemplate = path.join(generatedTemplateRoot, 'NovaOrynKernel.csproj');
        const kernelProjectPath = path.join(projectRoot, 'NovaOrynKernel.csproj');
        try {
            let projectText = await fs.readFile(kernelProjectPath, 'utf8');
            let projectChanged = false;
            if (!projectText.includes('Boot\\KernelPanicTransport.cs')) {
                projectText = projectText.replace(
                    '<Compile Include="Boot\\KernelTelemetryTransport.cs" />',
                    '<Compile Include="Boot\\KernelTelemetryTransport.cs" />\n    <Compile Include="Boot\\KernelPanicTransport.cs" />'
                );
                projectChanged = true;
            }
            if (!projectText.includes('Sdk\\NovaOryn.Kernel.X64.LowLevel\\NovaOryn.Kernel.X64.LowLevel.csproj')) {
                projectText = projectText.replace(
                    '<ProjectReference Include="Sdk\\NovaOryn.Freestanding.CoreLib\\NovaOryn.Freestanding.CoreLib.csproj" />',
                    '<ProjectReference Include="Sdk\\NovaOryn.Freestanding.CoreLib\\NovaOryn.Freestanding.CoreLib.csproj" />\n'
                    + '    <ProjectReference Include="Sdk\\NovaOryn.Kernel.X64.LowLevel\\NovaOryn.Kernel.X64.LowLevel.csproj" />'
                );
                projectChanged = true;
            }
            if (projectChanged) {
                await fs.writeFile(kernelProjectPath, projectText, 'utf8');
            }
        } catch {
            // ProjectCreator will recreate a missing kernel project file.
        }
