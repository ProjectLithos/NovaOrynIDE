...
export class NovaOrynWidget extends BaseWidget {
    ...
    protected renderStartup(): void {
        ...
        card.appendChild(this.element('h1', undefined, 'NovaOryn IDE 0.10.11'));
    ...
    protected renderConfiguration(): void {
        ...
        card.appendChild(this.element('h1', undefined, this.reconfiguringProjectPath ? `Reconfigure ${c.name}` : 'NovaOryn OS 0.10.11'));
    ...
    protected async deleteOperatingSystemSource(os: NovaOrynOperatingSystem): Promise<void> {
        ...
        // Remove the deleted OS from the visible list immediately. Do not wait for a
        // filesystem rescan before reflecting a successful destructive operation.
        this.operatingSystems = this.operatingSystems.filter(candidate =>
            candidate.path.toLowerCase() !== os.path.toLowerCase()
        );
        this.update();

        // Reconcile with the backend afterwards so the list remains authoritative if
        // another OS was added or removed while the deletion was in progress.
        await this.refreshOperatingSystems();
        await this.messages.info(`${os.name} OS #${os.instanceNumber} source code deleted and removed from the IDE list.`);
    }
