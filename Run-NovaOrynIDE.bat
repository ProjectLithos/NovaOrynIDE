...
echo [INFO] NovaOryn IDE Run 0.11.10
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$p='%NOVAORYN_BUILDSTATE%'; try { $j=Get-Content -LiteralPath $p -Raw | ConvertFrom-Json; if ([string]$j.novaOrynIdeVersion -ne '0.11.10') { exit 2 }; exit 0 } catch { exit 3 }"
...
echo [ OK ] Existing NovaOryn IDE 0.11.10 build is ready.
echo [INFO] Starting NovaOryn IDE 0.11.10...
...
:STALE_BUILD
echo [FAIL] The existing NovaOryn IDE build is not version 0.11.10 or its build-state marker is invalid.
echo [INFO] Run Build-NovaOrynIDE.bat once, then use Run-NovaOrynIDE.bat to launch it.
exit /b 1

:NOT_BUILT
echo [FAIL] NovaOryn IDE 0.11.10 has not been built completely yet.
