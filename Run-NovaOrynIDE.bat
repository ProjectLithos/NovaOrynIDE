...
echo [INFO] NovaOryn IDE Run 0.11.16
...
if not "%NOVAORYN_BUILT_VERSION%"=="0.11.16" goto STALE_BUILD
...
echo [ OK ] Existing NovaOryn IDE 0.11.16 build is ready.
echo [INFO] Starting NovaOryn IDE 0.11.16...
...
:STALE_BUILD
echo [FAIL] The existing NovaOryn IDE build is not version 0.11.16 or its generated-build marker is stale.
echo [INFO] Run Build-NovaOrynIDE.bat once, then use Run-NovaOrynIDE.bat to launch it.
exit /b 1

:NOT_BUILT
echo [FAIL] NovaOryn IDE 0.11.16 has not been built completely yet.
