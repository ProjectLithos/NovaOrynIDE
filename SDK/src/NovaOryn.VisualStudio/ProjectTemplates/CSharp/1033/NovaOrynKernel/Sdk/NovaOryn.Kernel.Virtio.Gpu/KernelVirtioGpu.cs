...
public static unsafe class KernelVirtioGpu
{
    ...
    {
        if(context==null||!KernelPci.TryGetDevice(context->Device,out PciDeviceInfo pci)||!IsGpu(pci))return false;Int32 slot=Free();if(slot<0){if(!Grow())return false;slot=Free();if(slot<0)return false;}DeviceRecord* r=_devices+slot;Clear((Byte*)r,(UInt64)sizeof(DeviceRecord));r->Used=1;r->DeviceHandle=context->Device.Value;r->Segment=pci.Location.Segment;r->Bus=pci.Location.Bus;r->PciDevice=pci.Location.Device;r->Function=pci.Location.Function;r->ResourceId=(UInt32)slot+1U;
