...
public static unsafe class KernelProcesses
{
    ...
    {
        internal UInt64 Id, Root, Entry, StackBase, StackTop; internal Int64 ExitCode; internal UInt32 State, Format, TableCount, AllocationCount;
...
#pragma warning restore CS0169
    ...
    public static Boolean TryCreateFromImage(UInt64 imageAddress, UInt64 imageLength, out KernelProcessInfo process)
    {
        ...
        r->Id=_nextId++; r->Root=root; r->Entry=executable.EntryPoint; r->State=(UInt32)KernelProcessState.Ready; r->Format=(UInt32)executable.Format; _active++;
    ...
    {
        ProcessRecord* r=Find(processId); if(r==null || r->State==(UInt32)KernelProcessState.Running)return false; Boolean ok=ReleaseOwned(r); r->ExitCode=exitCode; r->State=(UInt32)KernelProcessState.Terminated; if(_active!=0U)_active--; return ok;
    }
    ...
    private static Boolean LoadSegments(Byte* image, UInt64 length, ProcessExecutableInfo executable, UInt64 root, ProcessRecord* r, KernelPhysicalAllocation* tables, ref UInt32 tableCount)
    {
        ...
        {
            if(!ProcessExecutableMath.TryGetSegment(image,length,executable,s,out ProcessImageSegment segment))return false; UInt64 basePage=ProcessExecutableMath.PageFloor(segment.VirtualAddress);
    ...
    {
        UInt64 pages=DefaultStackBytes/4096UL; if(r->AllocationCount>=MaximumImageAllocations || !KernelPhysicalMemory.TryAllocate(pages,1UL,out KernelPhysicalAllocation allocation))return false; StoreAllocation(r,allocation); UInt64 stackBase=UserStackTop-DefaultStackBytes;
        ...
        r->StackBase=stackBase; r->StackTop=UserStackTop; return KernelProtectionMath.IsValidUserStack(r->StackTop);
    }
    ...
    private static KernelProcessInfo Snapshot(ProcessRecord* r)=>new(r->Id,(KernelProcessState)r->State,(ProcessExecutableFormat)r->Format,r->Root,r->Entry,r->StackBase,r->StackTop,r->ExitCode);
