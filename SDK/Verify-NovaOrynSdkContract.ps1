...
if ([string]$m.sdkVersion -ne '0.41.8') { Fail "SDK manifest version is $($m.sdkVersion); expected 0.41.8." }
...
foreach ($value in @('SdkVersion = "0.41.8"','ApiVersion = "1.2"','DriverAbiVersion = "1.0"')) {
    if (!$core.Contains($value)) { Fail "NovaOrynSdkContract is missing $value." }
}
Write-Host '[ OK ] NovaOryn SDK 0.41.8 stable API/ABI contract manifest verified.'
exit 0
