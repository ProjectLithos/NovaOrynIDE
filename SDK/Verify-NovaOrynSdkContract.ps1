...
if ([string]$m.sdkVersion -ne '0.40.0') { Fail "SDK manifest version is $($m.sdkVersion); expected 0.40.0." }
...
foreach ($value in @('SdkVersion = "0.40.0"','ApiVersion = "1.1"','DriverAbiVersion = "1.0"')) {
    if (!$core.Contains($value)) { Fail "NovaOrynSdkContract is missing $value." }
}
Write-Host '[ OK ] NovaOryn SDK 0.40.0 stable API/ABI contract manifest verified.'
exit 0
