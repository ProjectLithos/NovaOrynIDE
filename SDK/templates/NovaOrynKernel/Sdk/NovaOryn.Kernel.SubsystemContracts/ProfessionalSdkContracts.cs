...
public static class NovaOrynExecutableFormat { public const UInt32 Magic=0x50414F4EU; public const UInt16 Major=1; public const UInt16 Minor=0; public const String ApplicationExtension=".exe"; public const String NativeExecutableExtension=".nexe"; public const String DynamicLibraryExtension=".dll"; public const String StaticLibraryExtension=".lib"; }
...
public readonly struct NovaOrynExecutableMetadata { public NovaOrynExecutableMetadata(String id,String name,String version,String publisher,NovaOrynApplicationArchitecture architecture,UInt16 abiMajor,UInt16 abiMinor,NovaOrynApplicationAbi syscallAbi,UInt64 entryPointRva,String nativeImage,String[] dependencies,String[] requiredCapabilities,String[] resources){Id=id;Name=name;Version=version;Publisher=publisher;Architecture=architecture;AbiMajor=abiMajor;AbiMinor=abiMinor;SyscallAbi=syscallAbi;EntryPointRva=entryPointRva;NativeImage=nativeImage;Dependencies=dependencies;RequiredCapabilities=requiredCapabilities;Resources=resources;} public String Id{get;} public String Name{get;} public String Version{get;} public String Publisher{get;} public NovaOrynApplicationArchitecture Architecture{get;} public UInt16 AbiMajor{get;} public UInt16 AbiMinor{get;} public NovaOrynApplicationAbi SyscallAbi{get;} public UInt64 EntryPointRva{get;} public String NativeImage{get;} public String[] Dependencies{get;} public String[] RequiredCapabilities{get;} public String[] Resources{get;} }
public static class NovaOrynPackageContainer { public const String Extension=".zip"; public const String ManifestName="NovaOryn.Package.json"; public const String Format="novaoryn-package-v1"; public const UInt32 SchemaVersion=1; }
public enum NovaOrynPackageKind : Byte { Application=1,Driver=2,Library=3,Service=4,KernelExtension=5 }
public readonly struct NovaOrynPackageDependency { public NovaOrynPackageDependency(String id,String versionConstraint,Boolean optional){Id=id;VersionConstraint=versionConstraint;Optional=optional;} public String Id{get;} public String VersionConstraint{get;} public Boolean Optional{get;} }
public readonly struct NovaOrynPackageFile { public NovaOrynPackageFile(String path,UInt64 length,String sha256){Path=path;Length=length;Sha256=sha256;} public String Path{get;} public UInt64 Length{get;} public String Sha256{get;} }
public readonly struct NovaOrynPackageSignature { public NovaOrynPackageSignature(String state,String algorithm,String signerId,String manifestDigest,String value){State=state;Algorithm=algorithm;SignerId=signerId;ManifestDigest=manifestDigest;Value=value;} public String State{get;} public String Algorithm{get;} public String SignerId{get;} public String ManifestDigest{get;} public String Value{get;} }
public readonly struct NovaOrynPackageManifest { public NovaOrynPackageManifest(UInt32 schemaVersion,String id,String name,String version,NovaOrynPackageKind kind,String[] architectures,String minimumNovaOrynVersion,String sdkApiVersion,String abiVersion,NovaOrynPackageDependency[] dependencies,String[] capabilities,NovaOrynPackageFile[] files,NovaOrynPackageSignature signature){SchemaVersion=schemaVersion;Id=id;Name=name;Version=version;Kind=kind;Architectures=architectures;MinimumNovaOrynVersion=minimumNovaOrynVersion;SdkApiVersion=sdkApiVersion;AbiVersion=abiVersion;Dependencies=dependencies;Capabilities=capabilities;Files=files;Signature=signature;} public UInt32 SchemaVersion{get;} public String Id{get;} public String Name{get;} public String Version{get;} public NovaOrynPackageKind Kind{get;} public String[] Architectures{get;} public String MinimumNovaOrynVersion{get;} public String SdkApiVersion{get;} public String AbiVersion{get;} public NovaOrynPackageDependency[] Dependencies{get;} public String[] Capabilities{get;} public NovaOrynPackageFile[] Files{get;} public NovaOrynPackageSignature Signature{get;} }
...
public interface IKernelNetworkStackContract : IKernelNetworkingContract { Boolean TryGetLayerState(KernelNetworkLayer layer,out KernelSubsystemState state); Boolean TryGetNic(UInt64 interfaceHandle,out UInt64 deviceHandle,out UInt32 mtu,out UInt32 state); Boolean TryReceiveEthernet(UInt64 interfaceHandle,UInt64 frameAddress,UInt32 frameBytes); Boolean TryResolveArp(UInt64 interfaceHandle,UInt32 ipv4Address,out UInt64 macAddress); Boolean TryProcessNdp(UInt64 interfaceHandle,UInt64 ipv6PacketAddress,UInt32 packetBytes); Boolean TryConfigureIpv4(UInt64 interfaceHandle,UInt32 address,UInt32 prefix,UInt32 gateway); Boolean TryConfigureIpv6(UInt64 interfaceHandle,UInt64 addressHigh,UInt64 addressLow,UInt32 prefix); Boolean TrySendIcmpEcho(UInt64 interfaceHandle,UInt32 destination,UInt16 identifier,UInt16 sequence,UInt64 payloadAddress,UInt32 payloadBytes); Boolean TryCreateSocket(UInt32 type,out UInt64 socketHandle); Boolean TryBind(UInt64 socketHandle,UInt64 address,UInt16 port); Boolean TryConnect(UInt64 socketHandle,UInt64 address,UInt16 port); Boolean TryListen(UInt64 socketHandle,UInt32 backlog); Boolean TryAccept(UInt64 socketHandle,out UInt64 clientSocket); Boolean TrySendUdp(UInt64 socketHandle,UInt64 destination,UInt16 port,UInt64 bufferAddress,UInt32 bufferBytes); Boolean TryReceive(UInt64 socketHandle,UInt64 bufferAddress,UInt32 bufferBytes,out UInt32 received); Boolean TryResolveName(String host,out UInt64 address); Boolean TryResolveNameIpv6(String host,out UInt64 addressHigh,out UInt64 addressLow); }
