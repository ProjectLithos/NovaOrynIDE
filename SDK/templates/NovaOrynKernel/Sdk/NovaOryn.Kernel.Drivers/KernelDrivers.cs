...
public static unsafe class KernelDrivers
{
    ...
    private struct DriverRecord { internal Byte Used,State,Bus,MatchBus,MatchVendor,MatchDevice; internal UInt16 Vendor,Device; internal UInt32 ClassCode,ClassMask; internal UInt64 Probe,Start,Stop,Remove,Interrupt; }
    private struct DeviceRecord { internal Byte Used,State,Bus,Revision,ResourceCount; internal UInt16 Vendor,Device,SubsystemVendor,Subsystem; internal UInt32 ClassCode,Location,BoundDriver; internal fixed Byte ResourceType[ResourcesPerDevice]; internal fixed UInt64 ResourceStart[ResourcesPerDevice]; internal fixed UInt64 ResourceLength[ResourcesPerDevice]; internal fixed UInt64 ResourceFlags[ResourcesPerDevice]; }
    ...
    private static UInt64 _interruptRequestBroker,_interruptReleaseBroker;
    ...
    public static Boolean RegisterDriver(KernelDriverMatchRule rule,KernelDriverCallbacks callbacks,out KernelDriverHandle handle)
    {
        ...
        r->Probe=(UInt64)(void*)callbacks.Probe;r->Start=(UInt64)(void*)callbacks.Start;r->Stop=(UInt64)(void*)callbacks.Stop;r->Remove=(UInt64)(void*)callbacks.Remove;r->Interrupt=(UInt64)(void*)callbacks.Interrupt;_driverCount++;handle=new KernelDriverHandle((UInt32)slot+1U);return true;
    }
    ...
    { handle=default;if(_interruptRequestBroker==0UL||!KernelDriverMath.IsValidInterruptRequest(request)||!TryDevice(request.Device,out _))return false;delegate*<KernelDriverInterruptRequest*,KernelDriverInterruptHandle*,Boolean> broker=(delegate*<KernelDriverInterruptRequest*,KernelDriverInterruptHandle*,Boolean>)(void*)_interruptRequestBroker;KernelDriverInterruptHandle result=default;if(!broker(&request,&result))return false;handle=result;return true; }
