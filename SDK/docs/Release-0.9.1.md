...
The protection implementation intentionally uses the normal .NET `System.UInt64.MaxValue` member for overflow-safe user-range validation. The freestanding CoreLib supplied in 0.10.9 did not yet expose that standard primitive constant, causing the solution build to fail before NativeAOT compilation.
