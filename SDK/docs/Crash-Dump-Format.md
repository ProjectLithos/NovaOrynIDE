...
NovaOryn 0.10.1 makes crash dumps a public SDK format. The format is designed so newer IDE versions can continue opening older dumps and so non-IDE tools can consume the same crash information.
...
- Pre-0.10.1 IDE `schemaVersion: 1` JSON dumps are a legacy input format. NovaOryn IDE 0.10.1 retains an adapter for them; new producers must write the formal `NOCD` format.
...
```json
{
  ...
  "producer": { "product": "NovaOryn IDE", "version": "0.10.1" },
