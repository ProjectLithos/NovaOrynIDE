...
NovaOryn 0.10.2 defines a real, structured panic subsystem in `NovaOryn.Kernel.SubsystemContracts`.
...
When `writeCrashDump` is enabled the kernel emits a structured `[NOVAORYN:PANIC]` record. In a Debug session the panic debugger break stops QEMU through x64 `INT3`; NovaOryn IDE then captures the formal **NOCD 1.0** crash dump introduced in 0.10.2.
