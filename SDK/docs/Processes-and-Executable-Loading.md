...
`TryStart` switches CR3 to the process root and uses the native x64 `IRETQ` ring-3 transition. Once in user mode, the system-call boundary from 0.10.9 remains mapped through the shared kernel half, allowing a loaded program to execute `SYSCALL` and return with `SYSRET`.
