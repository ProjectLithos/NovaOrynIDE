...
const build=read('Build-NovaOrynIDE.bat'); const run=read('Run-NovaOrynIDE.bat'); const finalOrchestrator=read('CJS/Run-NovaOrynIDEFinalVerification.cjs'); ok(finalOrchestrator.includes('Verify-NovaOrynIDE01416.cjs'),'Final-verification orchestrator invokes the current 0.14.16 release verifier');
...
ok(finalOrchestrator.includes('Verify-NovaOrynIDE01416.cjs'),'Final-verification orchestrator invokes the current 0.14.16 release verifier');
ok(!build.includes('CJS\\Verify-NovaOrynIDE01410.cjs'),'Build does not invoke the previous 0.14.10 release verifier');
...
// 0.14.16: never trust a partial node_modules tree merely because Electron exists.
...
// 0.14.16: direct manifest verification is authoritative for hoisted workspace packages.
...
// 0.14.16: hoisted dev dependencies use the same authoritative direct-manifest verification as runtime dependencies.
...
// 0.14.16: the final verifier chain is orchestrated outside CMD so successful child verifiers cannot silently terminate the build.
...
ok(finalOrchestrator.includes('Verify-NovaOrynIDE0113.cjs') && finalOrchestrator.includes('Verify-NovaOrynIDETestFramework0110.cjs') && finalOrchestrator.includes('Verify-NovaOrynIDE01416.cjs'),'Final-verification orchestrator runs preset, SDK test-framework and current release verifiers');
