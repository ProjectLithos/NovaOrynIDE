...
const root=p.resolve(__dirname,'..'),read=x=>fs.readFileSync(p.join(root,x),'utf8');
...
let bad=false;for(const[c,ok]of checks){console.log(`${ok?'[ OK ]':'[FAIL]'} ${c}`);if(!ok)bad=true;}if(bad)process.exit(1);console.log('[ OK ] NovaOryn IDE 0.4.2 Image / Disk Explorer contract verified.');
