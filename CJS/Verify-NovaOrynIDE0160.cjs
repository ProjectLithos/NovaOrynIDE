...
const version=read('VERSION').split(/\r?\n/)[0].trim(); ok('release is 0.16.0',version==='0.16.0');
...
// Preserve the 0.15.1 automatic-console regression rule.
const toolbar=read('packages/novaoryn-ide/src/browser/novaoryn-toolbar-widget.tsx'); const runBody=(toolbar.match(/protected async runCurrentOperatingSystem\(\): Promise<void> \{[\s\S]*?\n    \}/)||[''])[0]; ok('Run/Debug still does not auto-attach Kernel Console',!runBody.includes('addWidget(this.kernelConsole')); ok('Run/Debug still uses bottom Output channel',runBody.includes('channel.show({ preserveFocus: false })'));
