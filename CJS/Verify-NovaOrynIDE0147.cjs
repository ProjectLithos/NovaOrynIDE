...
ok(build.includes('CJS\\Verify-NovaOrynIDE0147.cjs'),'Build invokes the current 0.14.7 release verifier');
...
ok(build.includes(installVerifyMarker) && build.includes('if errorlevel 1 ('),'Build checks the installed-dependency verifier exit code immediately');
...
ok(build.includes('no reinstall loop will be attempted'),'Build fails deterministically if the freshly installed dependency tree is invalid');
