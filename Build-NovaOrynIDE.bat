...
echo [INFO] NovaOryn IDE Build 0.4.7
...
echo [INFO] Verifying NovaOryn IDE 0.4.7 build toolchain...
...
if errorlevel 1 (
  echo [WARN] Installed dependency tree does not match the NovaOryn 0.4.7 pins.
  ...
  "%NOVAORYN_NODE%" "%~dp0Verify-NovaOrynIDEInstalledDependencies.cjs"
  if errorlevel 1 (
    echo [FAIL] Clean reinstall still does not match the NovaOryn 0.4.7 dependency pins.
    exit /b 2
  )
)
...
echo [INFO] Building NovaOryn IDE 0.4.7...
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$o=[ordered]@{ novaOrynIdeVersion='0.4.7'; theiaVersion='1.74.0'; electronVersion='42.3.0'; generatedUtc=(Get-Date).ToUniversalTime().ToString('o') }; $o | ConvertTo-Json | Set-Content -LiteralPath '%NOVAORYN_BUILDSTATE%' -Encoding UTF8"
if errorlevel 1 (
  echo [WARN] Build succeeded but NovaOryn could not record the dependency build-state marker.
)

echo [ OK ] NovaOryn IDE 0.4.7 build completed.
...
if errorlevel 1 (
  echo [INFO] Committing NovaOryn IDE 0.4.7 source changes.
  git commit -m "NovaOryn IDE 0.4.7"
  if errorlevel 1 goto :git_fail
) else (
