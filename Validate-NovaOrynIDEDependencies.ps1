...
try {
    ...
    foreach ($packageName in $ForbiddenRuntimePackages) {
        if ($electronPackage.dependencies.PSObject.Properties.Name -contains $packageName) {
            Fail "$packageName must not be a production dependency in 0.7.6. VS Code/Open VSX plugin loading is temporarily disabled to keep the vulnerable decompress chain out of the shipped runtime."
        }
    }
